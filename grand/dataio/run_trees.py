...
class MotherRunTree(DataTree):
    ...
    def get_run(self, run_no):
        ...
        if res == 0 or res == -1:
            logger.error(
                f"No run with run number {run_no}. Please provide a proper number."
            )
            return 0
...
class TRun(MotherRunTree):
    ...
    du_ground_tilt: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<float>")
    )
    """Angular tilt of the ground at the antenna"""
...
class TRunVoltage(MotherRunTree):
    ...
    digi_ctrl: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """Control parameters - the list of general parameters that can set the mode of operation, select trigger sources and preset the common coincidence read out time window (Digitizer mode parameters in the manual)."""
    # Firmware version
    firmware_version: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """Firmware version"""
    ...
    adc_sampling_frequency: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """ADC sampling frequency in MHz"""
    # ADC sampling resolution in bits
    adc_sampling_resolution: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """ADC sampling resolution in bits"""
    ...
    adc_input_channels: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """ADC input channels - > 16 BIT WORD (4*4 BITS) LOWEST IS CHANNEL 1, HIGHEST CHANNEL 4. FOR EACH CHANNEL IN THE EVENT WE HAVE: 0: ADC1, 1: ADC2, 2:ADC3, 3:ADC4 4:FILTERED ADC1, 5:FILTERED ADC 2, 6:FILTERED ADC3, 7:FILTERED ADC4. ToDo: decode this?"""
    # ADC enabled channels - LOWEST 4 BITS STATE WHICH CHANNEL IS READ OUT ToDo: Decode this?
    adc_enabled_channels: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """ADC enabled channels - LOWEST 4 BITS STATE WHICH CHANNEL IS READ OUT ToDo: Decode this?"""
    ...
    adc_conversion: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<float>")
    )
    """Conversion factor from bits to V for ADC"""
...
class TRunRawVoltage(MotherRunTree):
    ...
    trigger_position: StdVectorListDesc = field(
        default=StdVectorListDesc("unsigned short")
    )
    """Trigger position in the trace (trigger start = nanoseconds - 2*sample number)"""
    # Firmware version
    ...
    adc_input_channels_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned char>")
    )
    """ADC input channels for fv2: 0: ADC1, 1: ADC2, 2:ADC3, 3:ADC4, 4:FILTERED ADC1, 5:FILTERED ADC2, 6:FILTERED ADC3, 7:FILTERED ADC4, 15: off"""

    # ADC enabled channels - LOWEST 4 BITS STATE WHICH CHANNEL IS READ OUT
    adc_enabled_channels_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<bool>")
    )
    """ADC enabled channels - LOWEST 4 BITS STATE WHICH CHANNEL IS READ OUT"""
    ...
    pre_coincidence_window_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """Digitizer window parameters - describe Pre Coincidence, Coincidence and Post Coincidence readout windows (Digitizer window parameters in the manual). The unit is samples (to get time, multiply by 2)."""
    # Digitizer window parameters - describe Pre Coincidence, Coincidence and Post Coincidence readout windows (Digitizer window parameters in the manual). The unit is samples (to get time, multiply by 2).
    post_coincidence_window_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """Digitizer window parameters - describe Pre Coincidence, Coincidence and Post Coincidence readout windows (Digitizer window parameters in the manual). The unit is samples (to get time, multiply by 2)."""

    # Channel property parameters
    gain_correction_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """Channel property parameters"""
    integration_time_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    offset_correction_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned char>")
    )
    base_maximum_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    base_minimum_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )

    # Channel trigger parameters
    signal_threshold_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """Channel trigger parameters"""
    noise_threshold_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    # The unit is samples (to get time, multiply by 2)
    tper_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    """The unit is samples (to get time, multiply by 2)"""
    tprev_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    ncmax_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    tcmax_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    qmax_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    ncmin_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )
    qmin_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned short>")
    )

    notch_filters_no_ch: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<unsigned char>")
    )
...
class TRunEfieldSim(MotherRunTree):
    ...
    """Name of the atmospheric index of refraction model"""
    refractivity_model_parameters: StdVectorListDesc = field(
        default=StdVectorListDesc("double")
    )
...
class TRunShowerSim(MotherRunTree):
    ...
    hadronic_thinning_weight: TTreeScalarDesc = field(
...
class TRunNoise(MotherRunTree):
    ...
    gal_noise_sigma: StdVectorListDesc = field(
        default=StdVectorListDesc("vector<float>")
    )
    """Noise std dev for each arm of each antenna"""
